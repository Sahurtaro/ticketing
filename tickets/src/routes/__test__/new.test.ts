...
it('returns an error if an invalid price is provided', async () => {
  await request(app)
    ...
    .send({ title: 'sdafdas', price: -10 })
    .expect(400);

  await request(app).post('/api/tickets').set('Cookie', global.signin()).send({ title: 'dfafa' }).expect(400);
});
...
it('creates a ticket with valid inputs', async () => {
  ...
  await request(app).post('/api/tickets').set('Cookie', global.signin()).send({ title, price: 200 }).expect(201);
